...
//! ## Examples
//!
//! Run the migrations of the local runtime on the state of axiaaxc, from the axiaaxc repo where
//! we have `--chain axiaaxc-dev`, on the latest finalized block's state
//!
//! ```sh
...
//! block hash's state shall not yet have been pruned in `rpc.axiaaxc.io`.
//!
//! ```sh
//! RUST_LOG=runtime=trace,try-runtime::cli=trace,executor=trace \
//!     cargo run try-runtime \
...
//! specify a block hash. For the rest of these examples, we assume we're in the axiaaxc repo.
//!
//! First, let's assume you are in a branch that has the same spec name/version as the live axiaaxc
//! network.
//!
//! ```sh
//! RUST_LOG=runtime=trace,try-runtime::cli=trace,executor=trace \
...
//!
//! ```text
//! Request for native execution succeeded (native: axiaaxc-9900 (axia-axiaaxc-0.tx7.au0), chain: axiaaxc-9900 (axia-axiaaxc-0.tx7.au0))
//! ```
//!
...
//!     execute-block \
//!     --block-at <x> \
//!     --block-ws-uri wss://rpc.axiaaxc.io \
//!     --overwrite-wasm-code \
//!     snap \
//!     -s snap \
//! ```
