...
mod tests {
	...
	fn generate_key_respects_network_override() {
		let seed =
			generate_key::<sr25519::Pair>("ab", Ss58AddressFormatRegistry::AxiaaxcAccount.into())
				.unwrap();
		assert!(sr25519::Pair::from_seed_slice(&hex::decode(&seed[2..]).unwrap())
			.unwrap()
			.public()
			.to_ss58check_with_version(Ss58AddressFormatRegistry::AxiaaxcAccount.into())
			.contains("ab"));
	}
	...
	#[test]
	fn test_score_100() {
		let score = calculate_score("Axiaaxc", "5AxiaaxcwHY5k9GpdTgpqs9xjuNvtv8EcwCFpEeyEf3KHim");
		assert_eq!(score, 430);
	}
	...
	fn test_score_50_2() {
		...
		assert_eq!(
			calculate_score("Axiaaxc", "5PolkXXXXwHY5k9GpdTgpqs9xjuNvtv8EcwCFpEeyEf3KHim"),
			238
		);
	}
	...
	fn test_score_0() {
		assert_eq!(
			calculate_score("Axiaaxc", "5GUWv4bLCchGUHJrzULXnh4JgXsMpTKRnjuXTY7Qo1Kh9uYK"),
			0
		);
	}
